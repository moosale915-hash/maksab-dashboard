      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 space-y-4">
        <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center">
          <div className="relative flex-1 w-full lg:w-auto">
            ...
            <input
              type="text"
              placeholder="ابحث عن طلب (رقم أو منتج)..."
              className="w-full pl-4 pr-10 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-purple-300 outline-none"
              value={searchTerm}
                    ...
                    <td className="py-4 px-4">
                      <div className="flex gap-2">
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-gray-100 hover:bg-purple-100 hover:text-purple-700 transition-colors"
                          title="تفاصيل"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                        </button>
                        {order.tracking_url && (
                          <a href={order.tracking_url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg bg-gray-100 hover:bg-blue-100 hover:text-blue-700 transition-colors" title="تتبع الشحنة">
                    ...
                    {isExpanded && (
                      <td colSpan="7" className="bg-gray-50 p-4 border-t">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              ...
                              {items.map((item, idx) => (
                                <div key={idx} className="flex items-center justify-between bg-white p-2 rounded-lg">
                                  <div className="flex items-center gap-2">
                                    <img src={item.image_url || 'https://via.placeholder.com/40'} alt={item.name} className="w-10 h-10 rounded object-cover" />
                          ...
                          <div>
                            <h4 className="font-bold text-gray-700 mb-2">معلومات الشحن</h4>
                            <p className="text-sm text-gray-500">العنوان: الرياض، المملكة العربية السعودية</p>
                            <p className="text-sm text-gray-500 mt-1">رقم التتبع: {order.tracking_url ? 'متوفر' : 'غير متوفر'}</p>
                          </div>
                        </div>
                      </td>
                    )}
