...
export default function App() {
  ...
  const removeFromImportList = async (productId) => {
    ...
    setImportList((prevList) => prevList.filter((item) => item.id !== productId));
  ...
  const navigate = (page, data = null) => {
    ...
    if (paidPages.includes(page) && userSubscription?.plan_id === 'free') {
      showToast('هذه الصفحة متاحة فقط للمشتركين في الباقات المدفوعة. يرجى الترقية.', 'error');
      return;
    }
    ...
    if (isDashboard) {
      ...
      if (paidPages.includes(currentPage) && userSubscription?.plan_id === 'free') {
        return (
          <div className="flex flex-col items-center justify-center h-96 text-center p-8">
            ...
            <button onClick={() => navigate('subscription')} className="bg-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-purple-700">
      ...
      switch (currentPage) {
        case 'dashboard':
          return (
            <CustomerCatalog
              ...
              removeFromImportList={removeFromImportList}
        ...
        case 'settings':
          return <Settings />;
        case 'subscription':
          return <SubscriptionManagement onNavigate={navigate} isLoggedIn={isLoggedIn} />;
        case 'wallet':
          return <Wallet />;
        ...
        case 'payment':
          return <Payment 
            ...
            onPaymentSuccess={refreshUserSubscription}
          />;
        case 'integrations':
          return <Integrations onNavigate={navigate} />;
      ...
      switch (currentPage) {
        ...
        case 'catalog':
          return <PublicCatalog selectedCategory={catalogCategory} onNavigate={navigate} />;
        case 'pricing':
          return <PricingPage onNavigate={navigate} isLoggedIn={isLoggedIn} />;
        case 'services':
  ...
  return (
    <div className="flex h-screen bg-gray-50" dir="rtl">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <main className={isDashboard ? 'p-6 flex-1 overflow-auto' : ''}>
          {renderPage()}
        </main>
      </div>
