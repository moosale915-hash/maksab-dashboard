...
export default function PublicFooter({ onNavigate }) {
  const mainLinks = [
    ...
    { label: 'تصفح المنتجات', page: 'catalog' },
    { label: 'الباقات', page: 'pricing' },
    { label: 'كيف نعمل', page: 'how' },
    { label: 'من نحن', page: 'about' },
    { label: 'اتصل بنا', page: 'contact' },
    { label: 'الدعم الفني', page: 'support' },
  ];

  const legalLinks = [
    { label: 'سياسة الخصوصية', page: 'privacy' },
    { label: 'اتفاقية الاستخدام', page: 'terms' },
  ...
  const socialLinks = [
    ...
    ), bgColor: 'bg-gradient-to-tr from-[#833AB4] to-[#F77737]' },
    { name: 'تيك توك', url: '#', icon: (
    ...
    { name: 'سناب شات', url: '#', icon: (
      <img src="/images/snapchat-icon.png" alt="سناب شات" className="w-5 h-5" />
    ), bgColor: 'bg-[#FFFC00]' },
  ];

  return (
    <footer className="bg-gradient-to-r from-purple-600 to-indigo-700 text-white">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12">
        {/* اللوجو والروابط في تخطيط عمودي للموبايل */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 md:gap-0">
          {/* اللوجو */}
          <div className="flex justify-center md:justify-start">
            <img src="/images/logo.png" alt={APP_NAME} className="h-24 md:h-28 w-auto object-contain" />
          </div>

          {/* روابط سريعة - تتراص عمودياً في الموبايل */}
          <div className="flex flex-wrap justify-center gap-3 md:gap-6 text-xs md:text-sm">
            {mainLinks.map((link) => (
              <button key={link.label} onClick={() => onNavigate(link.page)} className="text-purple-100 hover:text-white transition-colors">
                {link.label}
              </button>
            ))}
          </div>
        ...
        {/* الروابط القانونية والأيقونات في صفوف منفصلة للموبايل */}
        <div className="mt-6 pt-6 border-t border-purple-400/30 flex flex-col md:flex-row md:justify-between items-center gap-4">
          <div className="flex flex-wrap justify-center gap-4 text-xs">
            {legalLinks.map((link) => (
              <button key={link.label} onClick={() => onNavigate(link.page)} className="text-purple-200 hover:text-white transition-colors">
                {link.label}
              </button>
            ))}
          </div>

          {/* أيقونات التواصل الاجتماعي - تتراص في صف واحد قابل للالتفاف */}
          <div className="flex flex-wrap justify-center gap-2">
            {socialLinks.map((social) => (
              <a
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center ${social.bgColor} hover:opacity-80 transition-opacity`}
              >
                {social.icon}
              </a>
            ))}
        ...
        {/* حقوق النشر */}
        <div className="text-center text-purple-300 text-xs mt-6">
